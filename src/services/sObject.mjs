...
import { SerializedResponse, NotFoundError } from "../common/serializedClass.mjs";
...
export async function getSObjectList(req, res) {
    ...
    const objectViews = await lazyloadListViews(objectName, headersTransform(req));
    const listView = objectViews.find((listView) => listView.developerName === viewName);
    if (!listView) {
        throw new NotFoundError(`List view ${viewName} not found`);
    }
    const url = listView.resultsUrl;
    const result = await read(url, headersTransform(req));
    return new SerializedResponse(result.data.records.map(parseListViewRecord), result.statusCode);
}